...
```
解析Excel(分档) → 当前选定 Agent 生成结构化动作计划 → 冻结 selection → 组内复用导航、行级执行 → 行级页面/结果复核 + 独立截图取证 → 每条立即落盘 → 生成 results.json → 异常行重规划/模块异常集中分析 → 画像候选反哺 → 通用回填器写回标注Excel → 记性价比(metrics)
```

- **解析Excel(分档)**：按分档口径（见 `tiering.md`）过滤出本轮范围，默认只取 `优先级==high`。
+ **模块/页面组规划**：`tools/module_planner.py` 只做 Excel/profile 事实归一化；当前选定 Agent 每个 Sheet/模块只读取一次上下文和相关 App 画像，输出 `agent_action_plan.json`（执行时归档进 `module_plan.json`）。用例按一级至四级目录、入口和前置条件生成连续页面组，不能只按“个股详情页”等页面名称跨市场合并。
- **按 Excel 行逐条执行**：同一页面组内可以复用导航，但仍按源文件行号逐条执行；公共导航方案可以复用，但不能合并业务动作、广播页面截图或依赖上一条残留状态。计划分组不改变 `source_order`；公共导航动作标记为 `setup`，不得混入某条用例的 `actual`。
+ **串行驱动手机**：确定性低层执行器用 `tools/droid.py` 一次一步执行 Agent 计划中的原语（adb + uiautomator），页面组首次进入时冷启动/恢复模块，组内后续行复用导航上下文但仍做 Agent 目标页门禁。目标页失败允许一次受限恢复导航，恢复失败才阻塞；执行器不得重新解析 Excel 操作描述。
...
| `module_planner.py` | 只归一化 Excel/profile 事实，生成 Agent 输入上下文；不负责把自然语言翻译成动作。 |
| `agent_plan.py` | 生成 Agent 上下文、校验逐行 `navigation/actions/target_page` 计划，并绑定源 Excel SHA-256。 |
