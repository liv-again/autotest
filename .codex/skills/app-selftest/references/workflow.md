...
```
解析Excel(分档) → 模块/页面组规划 → 冻结 selection → 组内复用导航、行级执行 → 行级页面/结果复核 + 独立截图取证 → 每条立即落盘 → 生成 results.json → 异常行重规划/模块异常集中分析 → 画像候选反哺 → 通用回填器写回标注Excel → 记性价比(metrics)
```
...
- **串行驱动手机**：确定性执行器用 `tools/droid.py` 一次一步操作真机（adb + uiautomator）；页面组首次进入时冷启动/恢复模块，组内后续行复用导航上下文但仍做目标页门禁。目标页失败允许一次运行时重规划，恢复失败才阻塞。
