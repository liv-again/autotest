...
```
解析Excel(分档) → 按"屏"分组用例 → 串行驱动手机(一屏多用例) → 元素树断言 + 截图取证 → 回填tested字典 → 重生成标注Excel → 记性价比(metrics)
```
...
- **回填 tested 字典 → 重生成标注Excel**：结果写进 `tools/annotate_excel.py` 的 `tested` 字典，重跑后在原 Excel 右侧追加 AI 列 + 内联截图。
...
| `annotate_excel.py` | 把 `tested` 结果字典写回 Excel，追加 AI 列 + 内联截图；含分档函数 `tierof()`。 |
