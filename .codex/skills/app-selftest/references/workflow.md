...
```
解析Excel(分档) → 当前选定 Agent 生成结构化动作计划 → 冻结 selection → 组内复用导航、行级执行 → 行级页面/结果复核 + 独立截图取证 → 每条立即落盘 → 生成 results.json → 阻塞用例独立会话逐条 LLM 复测（可选）→ 异常行/模块异常集中分析 → 画像候选反哺 → 通用回填器写回标注Excel → 记性价比(metrics)
```
...
- **串行驱动手机**：首轮确定性低层执行器用 `tools/droid.py` 一次一步执行已校验 Agent 计划中的原语；目标页失败最多消费一次计划内 `recovery_navigation`。阻塞复测若启用 `--llm-retest`，则由独立 retester 会话重新读取原始 Excel、画像和实时截图/UI 树，逐步选择一个动作；旧计划只作审计参考。
