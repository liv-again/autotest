...
│   ├── annotate_excel.py       #   通用结果回填 CLI/API：按 ID/行号/名称写回 + 内联证据
...
解析Excel(分档) → 按"屏"分组用例 → 串行驱动手机(一屏多用例)
  → 元素树断言(droid has/find) + 截图取证(shot) → 生成结构化 results.json
  → annotate_excel.py 回填标注 Excel → metrics.py 记性价比
```
...
| `requirements.txt` | `PyYAML>=6.0`、`jsonschema>=4.0`、`pytest>=8.0`、`openpyxl>=3.1`。                                                                 |
...
| 文件                  | 作用                                                                                                                                                                                                                                                                                                                                  |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `droid.py`          | **AI 操作真机的唯一入口**（零第三方依赖，直接 subprocess 调 adb，规避 Git Bash 路径改写）。子命令：`current`（前台 activity）/ `screen`（dump 元素树打印可交互元素）/ `find "文字"`（定位元素+坐标）/ `has "词"...`（断言，退出码 0/1，**断言首选**）/ `tap --text/--id/--xy/--bbox`（视觉框中心点击）/ `type`（仅数字/ASCII，中文不行）/ `key BACK...` / `swipe` / `shot` / `dump-xml`（保存 UI 树，截图 0 字节会提示 FLAG_SECURE）。设 `DROID_LOG` 环境变量即记动作日志供 metrics 统计。 |
| `execution_gate.py` / `execution_policy.yaml` | **全局执行完整性门禁**：适用于所有 App、Sheet、模块和用例。模块级规划只做一次，正式执行按 Excel 行逐条进行；要求执行器声明 `execution_manifest`，每条已执行用例提供 `action_trace → observation → evidence`；允许复用导航方案，但禁止合并业务动作或广播组级证据；未执行/待跑、缺动作轨迹、缺独立证据或选中范围不完整时拒绝严格结果。 |
| `agent_binding.py` / `agent_session.py` | **规划、复测、复核的角色绑定与会话隔离**：默认继承同一 Agent/model，但 planner、每条 retester 和只读 reviewer 使用不同会话；桌面宿主通过 `SIXGILL_AGENT_SESSION_FACTORY=module:function` 接入，核心不启动 CLI 或 provider 专用传输。 |
| `module_planner.py` / `execution_journal.py` / `exception_queue.py` | **模块规划、逐条落盘、异常集中分析**：LLM 每个模块只解析一次，确定性执行器按 Excel 行消费计划并即时写入 JSONL/checkpoint；首轮运行时异常按计划内恢复规则处理并留存证据，阻塞行可在延迟复测队列中显式启用 `--llm-retest`，由一个长期复用的 Queue 级 retester 会话在每条 Case 开始时读取原始用例、画像和实时截图/UI 树，一次生成完整动作计划；模块完成后由独立只读 reviewer 会话复核结果。 |
| `build_results.py` | **逐步骤结果构建器与质量门**：只接收执行器提供的 `action → observation → status` 事实，不从操作轨迹生成默认 `actual`；校验步骤定位字段、空/通用/仅复述 action 的结果、跨用例大量复用和 evidence，并调用全局执行完整性门禁，保留顶层 `setup_trace`，输出结果契约 `2.0`。 |
| `retest_results.py` / `_run_three_sheets.py --retest-queue` | **未通过用例逐条复测器**：`retest_results.py plan` 生成单用例复测队列，`_run_three_sheets.py --retest-queue <retest_queue.json>` 只执行队列中的行并为每条重新 setup，同时生成 `retest_execution.json`；默认兼容模式重放已校验计划，显式 `--llm-retest` 时改由每条用例一个独立 retester 会话重新理解和执行，旧计划仅作审计参考；再用 `retest_results.py merge` 合并，最终状态采用第二轮，并在 `attempts` 中保留首轮与复测完整记录。 |
| `annotate_excel.py` | **通用结果回填器**：读取任意 `.xlsx/.xlsm` 用例文件和 JSON/YAML 结果；按 `row/source_row`、已有 `case_id`、或 `sheet+case_name` 定位，重复名称的非连续区块拒绝猜测；支持 `steps` 步骤级结果，同一行多步骤按序换行回填，跨行步骤只写入各自源行，不再广播整块结果；右侧幂等追加/更新 `🤖AI` 列，状态通用着色，证据图片内联嵌入，输出当前批次的 `🤖AI自测汇总`。支持 `annotate_workbook()` API、`--results`/`--results-json` CLI、`--strict` 结果质量、执行完整性与未匹配保护，默认不覆盖源文件。 |
...
# 每个完整执行批次必须随结果提交 execution_manifest；
# 严格构建会校验 selected_cases 数量、逐条 action_trace、observation 和独立证据。
...
python tools/annotate_excel.py --src cases.xlsx --results results.json --out cases_AI自测结果.xlsx --strict
...
2. **`annotate_excel.py` 仅支持 `.xlsx/.xlsm`**；`.xls` 需先另存为新版 Excel 格式。结果定位不唯一时必须补充 `row/source_row`，避免同名用例误回填；多步骤结果应使用 `steps[].step_id` 和步骤级 `row/source_row`，否则不会向连续行广播。`metrics.py` 仍有历史 `PROJECT_DIR` 默认路径，可用 `CC_TRANSCRIPT` 环境变量覆盖。
...
需要新建的只有（**均已落地，2026-08-16**）：

| 新件 | 内容 | 状态 |
|---|---|---|
| `tools/init_app.py` | slug/package/version → 过 schema 的空骨架 + 白标种子 | ✅ 已实现（含 10 项 pytest） |
| skill 里的 explore mode 段落 | 8.5.2–8.5.7 的编排规程（含 8.5.4 导航图爬取 + 本节轨一/轨二入口判断） | ✅ 已落盘为 `.claude/skills/app-selftest/references/explore.md`，并接线 SKILL.md Step 2 与 onboarding.md B 节 |
