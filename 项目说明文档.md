...
| `module_planner.py` / `execution_journal.py` / `exception_queue.py` | **模块规划、逐条落盘、异常集中分析**：LLM 每个模块只解析一次，确定性执行器按 Excel 行消费计划并即时写入 JSONL/checkpoint；首轮运行时异常按计划内恢复规则处理并留存证据，阻塞行可在延迟复测队列中显式启用 `--llm-retest`，由独立 retester 会话重新读取原始用例、画像和实时截图/UI 树，逐步决定动作；模块完成后由独立只读 reviewer 会话复核结果。 |
