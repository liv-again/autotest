...
| `module_planner.py` / `execution_journal.py` / `exception_queue.py` | **模块规划、逐条落盘、异常集中分析**：LLM 每个模块只解析一次，确定性执行器按 Excel 行消费计划并即时写入 JSONL/checkpoint，模块完成后只将异常用例交给 LLM 分析。 |
