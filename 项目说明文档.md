...
| `build_results.py` | **逐步骤结果构建器与质量门**：只接收执行器提供的 `action → observation → status` 事实，不从操作轨迹生成默认 `actual`；校验步骤定位字段、空/通用/仅复述 action 的结果、跨用例大量复用和 evidence，保留顶层 `setup_trace`，输出结果契约 `2.0`。 |
| `retest_results.py` | **未通过用例逐条复测器**：sheet/模块首轮完成后生成单用例复测队列，要求每条复测重新 setup；合并时最终状态采用第二轮，并在 `attempts` 中保留首轮与复测完整记录。 |
| `annotate_excel.py` | **通用结果回填器**：读取任意 `.xlsx/.xlsm` 用例文件和 JSON/YAML 结果；按 `row/source_row`、已有 `case_id`、或 `sheet+case_name` 定位，重复名称的非连续区块拒绝猜测；支持 `steps` 步骤级结果，同一行多步骤按序换行回填，跨行步骤只写入各自源行，不再广播整块结果；右侧幂等追加/更新 `🤖AI` 列，状态通用着色，证据图片内联嵌入，输出当前批次的 `🤖AI自测汇总`。支持 `annotate_workbook()` API、`--results`/`--results-json` CLI、`--strict` 结果质量与未匹配保护，默认不覆盖源文件。 |
...
python tools/annotate_excel.py --src cases.xlsx --results results.json --out cases_AI自测结果.xlsx
