...
| `module_planner.py` / `execution_journal.py` / `exception_queue.py` | **模块规划、逐条落盘、异常集中分析**：LLM 每个模块只解析一次，确定性执行器按 Excel 行消费计划并即时写入 JSONL/checkpoint；运行时异常进入当前行的用例级 Agent 接管，同一行后续出现新的阻塞会继续调用 Agent，成功恢复的诊断、动作、截图和轨迹写入 `profile_feedback.json` 候选，模块完成后再将异常用例交给 LLM 分析。 |
...
| `retest_results.py` / `_run_three_sheets.py --retest-queue` | **未通过用例逐条复测器**：`retest_results.py plan` 生成单用例复测队列，`_run_three_sheets.py --retest-queue <retest_queue.json>` 只执行队列中的行并为每条重新 setup，同时生成 `retest_execution.json`；再用 `retest_results.py merge` 合并，最终状态采用第二轮，并在 `attempts` 中保留首轮与复测完整记录。 |
