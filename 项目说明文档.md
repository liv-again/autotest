...
> 本文基于对仓库全部源码、配置、文档的逐一阅读整理（2026-08-09 初版；2026-08-13 刷新至 `runs/2026-08-10-test-cases`、profile.yaml 258 行版），测试套件实测 **150 passed / 1 failed**（失败为既有问题，见 §七-8）。
...
├── tests/                      # pytest 套件（151 个测试）+ fixtures（valid/invalid 样本）
...
### 4.8 `tests/` —— 测试体系（150 passed / 1 failed）
...
> ⚠️ 当前 1 个既有失败：`tests/apps/test_guojin_profile.py::test_profile_schema_valid`——2026-08-10 手写新增的 `cap.industry_panel` 携带 `app_version`/`evidence_run`，但 profile schema 的 capabilities 节未声明这两字段（entries 节有、capabilities 节无，属不对称）。修法二选一：删该条目的两字段，或给 capabilities schema 补上（推荐后者，与 entries 对齐，保留版本追溯）。详见 §七-8。
...
8. **当前测试 1 红**：`test_profile_schema_valid` 失败——`cap.industry_panel`（2026-08-10 手写新增）携带 `app_version`/`evidence_run`，超出 capabilities schema（`additionalProperties:false` 且未声明这两字段；entries 节有、capabilities 节无）。教训：画像写盘必须走 `reback_run` 的 schema 门，手写绕过即翻车。
