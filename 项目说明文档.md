...
| `annotate_excel.py` | **通用结果回填器**：读取任意 `.xlsx/.xlsm` 用例文件和 JSON/YAML 结果；按 `row/source_row`、已有 `case_id`、或 `sheet+case_name` 定位，重复名称的非连续区块拒绝猜测；支持 `steps` 步骤级结果，同一行多步骤按序换行回填，跨行步骤只写入各自源行，不再广播整块结果；右侧幂等追加/更新 `🤖AI` 列，状态通用着色，证据图片内联嵌入，输出当前批次的 `🤖AI自测汇总`。支持 `annotate_workbook()` API、`--results`/`--results-json` CLI、`--strict` 未匹配保护，默认不覆盖源文件。 |
