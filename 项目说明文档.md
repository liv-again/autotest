...
| `annotate_excel.py` | **通用结果回填器**：读取任意 `.xlsx/.xlsm` 用例文件和 JSON/YAML 结果；按 `row/source_row`、已有 `case_id`、或 `sheet+case_name` 定位，重复名称的非连续区块拒绝猜测；右侧幂等追加/更新 `🤖AI` 列，状态通用着色，证据图片内联嵌入，输出当前批次的 `🤖AI自测汇总`。支持 `annotate_workbook()` API、`--results`/`--results-json` CLI、`--strict` 未匹配保护，默认不覆盖源文件。 |
...
2. **`annotate_excel.py` 仅支持 `.xlsx/.xlsm`**；`.xls` 需先另存为新版 Excel 格式。结果定位不唯一时必须补充 `row/source_row`，避免同名用例误回填。`metrics.py` 仍有历史 `PROJECT_DIR` 默认路径，可用 `CC_TRANSCRIPT` 环境变量覆盖。
